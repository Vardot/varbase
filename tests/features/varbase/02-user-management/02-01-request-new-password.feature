  ...
  Scenario: Verify that the system cannot send an email to non-existing users
    Given I am not logged in
     ...
     When I fill in "testuser" for "Username or email address"
      ...
      And I wait 3s
     Then I should see "If testuser is a valid account, an email will be sent with instructions to reset your password." in the "div" element with the "aria-label" attribute set to "Status message"
